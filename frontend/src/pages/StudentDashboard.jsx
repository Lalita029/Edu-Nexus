...
const StudentDashboard = () => {
  ...
  const fetchChaptersAndQuizzes = async () => {
    ...
    try {
      // 1. Fetch chapters
      const chRes = await apiFetch(`/api/student/chapters/${selectedSubject}`, {
      ...
      // 2. Fetch quizzes
      const qRes = await apiFetch(`/api/student/quizzes/${selectedSubject}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const fetchLeaderboard = async () => {
    setLoading(true);
    try {
      const res = await apiFetch('/api/student/leaderboard', {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const submitQuiz = async (finalAnswers) => {
    setLoading(true);
    try {
      const res = await apiFetch(`/api/student/quizzes/${activeQuiz._id}/submit`, {
        method: 'POST',
        headers: {
